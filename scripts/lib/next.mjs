...
const VERB_FOR = {
  ...
  review: "the loop: `gate brief reviewer`, spawn done-gate:reviewer with the prompt it prints, `gate huddle add reviewer --file review-<n>.md`; at size small fix the items yourself and `gate huddle resolve H<k>.<i> --evidence <ptr>`; at standard and up `gate brief worker`, SendMessage the worker the review file's path (or spawn done-gate:worker with the prompt), then `gate huddle reply --file worker-<n>.md` when it answers (fixed: closes, disagree: disputes); repeat with the same reviewer (SendMessage it the next packet) until nothing is open, at most three rounds; what is still disputed after round three goes to `gate brief arbiter --item H<k>.<i>` and done-gate:arbiter; then `gate step review done … --evidence review-<n>.md`; you own the diff and the summary, the reviewer's file is embedded as it wrote it",
  close: "`gate close`",
};
