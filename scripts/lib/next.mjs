...
import { optionalSteps, tiered, tierOf } from "./size.mjs";
...
const VERB_FOR = {
  ...
  skeptic: "`gate brief skeptic`, spawn done-gate:skeptic with the prompt it prints, then `gate huddle add skeptic --summary \"…\"` and `gate step skeptic done … --evidence ledger.json#huddles`",
  ...
  driver: "drive the real surface (phone viewport first), then `gate decide driver …` and `gate step driver done … --evidence decisions#<n>` (or `na` when no UI changed)",
  schema: "probe the real schema, then `gate step schema done \"<what you ran>\" --evidence <ptr>` (or `na` when no schema file changed)",
  ...
  review: "`gate brief reviewer`, spawn done-gate:reviewer with the prompt it prints, then `gate huddle add reviewer --file review-<n>.md`, one `gate huddle acton` per Act-on item, fix, `gate huddle resolve`, then `gate step review done … --evidence review-<n>.md`",
...
export function nextHint(ledger) {
  ...
  const reopened = new Set(tierOf(ledger).reopened);
  const blank = ledger.steps.filter((s) => !s.state);
  const step = blank.find((s) => s.key && reopened.has(s.key)) ?? blank[0];
  ...
  if (step) {
    ...
    const why = step.key && reopened.has(step.key) ? " (reopened: the task outgrew its size)" : "";
    return `next: step ${step.n}${why}: ${how ?? `${step.text} then \`gate step ${step.n} done|skipped|na "…"\``}`;
  }
