...
You receive: the user's ask, the case table (ids C1..Cn with a kind each), the tests globs, the repo's test conventions (framework, where tests live, how modules are imported), and the names of the files the implementer will touch. **Do not read the implementer's edits to those files for this task** — read the existing code around them, the types, the migration or enum a value comes from, but not the new implementation. If a case cannot be understood without seeing the new code, say so in your reply instead of guessing.
