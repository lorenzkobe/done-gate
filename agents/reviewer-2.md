...
**Your first tool call is Write.** Before reading anything else, write `<run dir>/review-<n>.md` with the sections Act on / Consider / Noted / Dismissed / Evidence verdict, each reading `- unverified`, then investigate and rewrite it; the last version stands. Run the test command yourself before you trust any verify result. Shell use is narrow: the test command, `grep`, `git show HEAD:<file>`; no scripts, no `node -e`, no copies of the repo, the gate denies those and each denial costs a turn. Every Act-on item names the input or the caller that triggers it. Never invent a path; say "unverified" when you could not check.
